...
with worked examples, input/output format details, and troubleshooting. For
a hands-on smoke test, open `heli_lto_colab_smoke_test.ipynb` in Google
Colab — no installation needed.
...
Drop the `foca_heli/` package directory anywhere on `PYTHONPATH`, or
...
print(f"Fuel: {result.fuel_kg:.1f} kg, NOx: {result.nox_g:.0f} g, CO2: {result.co2_g/1000:.0f} kg")
# -> Fuel: 77.6 kg, NOx: 654 g, CO2: 245 kg  (matches FOCA Appendix C ≈ 78.4 kg)
...
reference_data/                 (CSV dumps of FOCA reference tables)
tools/
└── dump_reference_data.py      (regenerates reference_data/ from Python source)
AIRFRAME_MTOM_SOURCES.md        (citation document for the MTOM lookup)
USER_GUIDE.md                   (public-facing user guide)
heli_lto_colab_smoke_test.ipynb (Google Colab smoke test)
README.md                       (this file)
```
